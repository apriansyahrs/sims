...
class Kelas_model extends CI_Model {
	...
	public function getKelasList($tp, $smt) {
        ...
        //$this->db->join('buku_induk i', 'i.id_siswa=e.id_siswa AND =i.status=1');
    ...
    public function getKelasByNama($nama_kelas, $id_tp=null, $id_smt=null) {
        ...
        $this->db->join('master_guru g', 'g.id_guru=f.id_guru', 'left');
        $this->db->join('master_siswa si', 'si.id_siswa=k.siswa_id', 'left');
        //$this->db->join('buku_induk i', 'i.id_siswa=k.id_siswa AND =i.status=1');
        $this->db->order_by('nama_kelas', 'ASC');
    ...
    /*
    public function save($tp, $smt) {
		$rows = count($this->input->post('siswa', true));
		$siswakelas = [];
		for ($i = 0; $i <= $rows; $i++) {
				$siswakelas[] = [
				'id' => $this->input->post('siswa['.$i.']', true)
			];
		}
		$jumlah = serialize($siswakelas);
		$data = array(
			'nama_kelas' => strip_tags($this->input->post('nama_kelas', TRUE)),
            'kode_kelas' => strip_tags($this->input->post('kode_kelas', TRUE)),
			'jurusan_id' => strip_tags($this->input->post('jurusan_id', TRUE)),
			'id_tp' => $tp,
			'id_smt' => $smt,
			'level_id' => strip_tags($this->input->post('level_id', TRUE)),
			'guru_id' => strip_tags($this->input->post('guru_id', TRUE)),
			'siswa_id' => strip_tags($this->input->post('siswa_id', TRUE)),
			'jumlah_siswa' => $jumlah//strip_tags($this->input->post('jumlah_siswa', TRUE)),
		);
	...
	public function update($id, $tp, $smt) {
		$rows = count($this->input->post('siswa', true));
		$siswakelas = [];
		for ($i = 0; $i <= $rows; $i++) {
			$siswakelas[] = [
		...
		return $this->db->update('master_kelas', $data);
	}
    */
	...
	public function getAllSiswa($tp, $smt) {
        ...
        //$this->datatables->join('users e', 'a.username=e.username');
	...
	public function getMateriKelasSiswa($id_kjm, $jenis) {
		$this->db->select('a.id_kjm, a.id_materi, a.jadwal_materi, a.jurnal, b.*, c.nama_guru, c.foto, e.id_mapel, e.nama_mapel, d.mapel_kelas as kelas_guru');
		$this->db->from('kelas_jadwal_materi a');
	...
	public function getAllKodeMateri($id_tp, $id_smt, $id_guru=null) {
		...
		/*
		$this->db->select('a.id_kjm, a.id_materi, b.kode_materi, b.materi_kelas, b.id_guru');
        $this->db->from('kelas_jadwal_materi a');
		if ($id_guru==null) {
			$this->db->join('kelas_materi b', 'a.id_materi=b.id_materi');
		} else {
			$this->db->join('kelas_materi b', 'a.id_materi=b.id_materi AND b.id_guru='.$id_guru);
		}
	...
	public function getMateriSiswa($id_kelas, $tgl, $jenis) {
        ...
        if ($result) {
            foreach ($result as $key => $row) {
                //id_materi = id_kelas + tp + smt + tahun + BULAN + tgl + jam_ke + jenis
	...
	public function getRekapBulananMapel($id_siswa, $bulan) {
		$this->db->select('a.log_time as materi, DAYOFMONTH(a.log_time) as tanggal, MONTH(a.log_time) as bulan, YEAR(a.log_time) as tahun, TIME_FORMAT(a.log_time, "%H:%i") as jam_materi');//, b.log_time as tugas, TIME_FORMAT(b.log_time, "%H:%i") as jam_tugas');
		$this->db->from('log_materi a');
	...
	public function getRekapBulananMateri($id_siswa, $date, $id_materi) {
		...
		$this->db->from('log_materi a');
		$this->db->where('DATE(a.log_time)', $date);
		//$this->db->where('MONTH(a.log_time)', $date);
		$this->db->where('a.id_siswa', $id_siswa);
		//$this->db->where('a.log_type', '2');
		$this->db->where('a.id_materi', $id_materi);
	...
	public function getRekapMateriSemester($id_kelas, $id_materi = null) {
		...
		//$this->db->where('MONTH(log_time)', $bulan, false);
		//$this->db->where('YEAR(log_time)', $tahun, false);
        //$this->db->where('log_type', '2');
        ...
        if ($result) {
            foreach ($result as $key => $row) {
                //id_materi = id_kelas 3 + tp 1 + smt 1 + tahun 4 + BULAN 2 + tgl 2 + jam_ke 2?3 + jenis 1
                $len_kls = strlen($id_kelas); //3
                $len_tp_smt = 2;
                $len_tahun = 4;
                $len_bln = 2;
                $len_hari = 2;

                $subs_bln = $len_kls + $len_tp_smt + $len_tahun; // 9
                $subs_tgl = $subs_bln + $len_bln; // 11
                $sisa = strlen($row->id_materi) - ($len_kls + 10); // (16-11) 5
                $len = $sisa === 3 ? 2 : 1; //
    ...
    public function getCatatanMapelBySiswa($id_kelas, $id_tp, $id_smt){
        ...
        //$this->db->where('a.id_kelas', $id_kelas);
        //$this->db->or_where('a.id_siswa', $id_siswa);
        $this->db->where('a.id_tp', $id_tp);
        $this->db->where('a.id_smt', $id_smt);
        return $this->db->get()->result();
    }

...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#users").on("click", ".btn-nonaktif", function() {
            ...
            $('#loading').removeClass('d-none');
            $.ajax({
                url: base_url + "userorangtua/deactivate/" + username + "/" + nama,
                type: "GET",
                success: function(response) {
                    $('#loading').addClass('d-none');
                    if (response.msg) {
        ...
        $("#users").on("click", ".btn-reset", function() {
            ...
            $('#loading').removeClass('d-none');
            $.ajax({
                url: base_url + "userorangtua/reset_login/" + username + "/" + nama,
                type: "GET",
                success: function(response) {
                    $('#loading').addClass('d-none');
                    if (response.msg) {
        ...
        $(".btn-action").on("click", function() {
            ...
            let uri = action === 'aktifkan' ? base_url + "userorangtua/aktifkansemua" : base_url + "userorangtua/nonaktifkansemua";
    ...
    function loadOrangtua() {
        ...
        $.ajax({
            url: base_url + 'userorangtua/list',
            data: dataPost,
            type: 'POST',
    ...
    function previewData(data) {
        ...
        if (data.lists.length > 0) {
            $.each(data.lists, function(idx, orangtua) {
                html += `<tr>
                <td class="text-center align-middle">
                    <input type="hidden" name="ids[]" value="${orangtua.unique_id}">
                    ${Number((perPage * (currentPage - 1)) + (idx + 1))}</td>
            <td class="align-middle">${orangtua.nama_siswa}</td>
            <td class="align-middle">${orangtua.nama_orang_tua}</td>
            <td class="align-middle">${orangtua.status_keluarga}</td>
            <td class="align-middle">${orangtua.username}</td>
            <td class="align-middle">${orangtua.username}</td>
            <td class="text-center align-middle">
                <button type="button" class="btn btn-default btn-xs btn-reset" ${orangtua.reset == '0' ? 'disabled' : ''} data-username="${orangtua.username}" data-nama="${orangtua.nama_orang_tua}" data-toggle="tooltip" title="Reset Login"> <i class="fa fa-sync text-xs mr-1 ml-1"></i></button>
            </td>
            <td class="text-center align-middle p-1">`;
                if (orangtua.aktif == "0") {
                    html += `<span class="badge badge-danger">Nonaktif</span><br><button type="button" class="btn btn-aktif btn-success btn-xs" data-id="${orangtua.unique_id}" data-toggle="tooltip" title="Aktifkan"> <i class="fa fa-user-plus text-xs mr-1 ml-1"></i> </button>`;
                } else {
                    html += `<span class="badge badge-success">Aktif</span><br><button type="button" class="btn btn-nonaktif btn-danger btn-xs" data-username="${orangtua.username}" data-nama="${orangtua.nama}" data-toggle="tooltip" title="Nonaktifkan"> <i class="fa fa-ban text-xs mr-1 ml-1"></i></button>`;
                }
                html += `</td></tr>`;
            });
        } else {

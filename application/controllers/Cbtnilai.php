...
/*   ________________________________________
    ...
    |________________________________________|
*/
class Cbtnilai extends MY_Controller
{
            ...
            foreach ($siswas as $siswa) {
                ...
                foreach ($logs as $log) {
                    if ($log->id_siswa == $siswa->id_siswa) {
                        ...
                        $loading = $sudahSelesai ? '<i class="fa fa-check"></i> ' :
                            ($sudahMulai ? '<i class="fa fa-spinner fa-spin"></i> ' : '');
                    }
                }
                ...
                if ($info->tampil_pg > 0) {
                    if (count($jawaban_pg) > 0) {
                        foreach ($jawaban_pg as $num => $jwb_pg) {
                            ...
                            $arrJawabanPg[$num] = [
                                'jawaban' => strtoupper($jwb_pg->jawaban_siswa ?? ''),
                                'benar' => $benar
                            ];
                        }
                    } else {
                        ...
                        if (isset($jawab_jod->jawaban_siswa->links)) {
                            ...
                            foreach ($array1 as $key => $subArray1) {
                                ...
                                if (isset($array2[$key])) {
                                    ...
                                    //$point_soal += $point_subitem * count($sameItems);
                                    ...
                                    //$item_salah += count($diffItems1) + count($diffItems2);
                                    $item_kurang += count($diffItems1) + count($diffItems2);
                                    $arrBenar[$key]->kurang += count($diffItems1);
                                    //$arrBenar[$key]->salah += count($diffItems1) + count($diffItems2);
                                } else {
                ...
                if ($ya != null) {
                    ...
                    $siswa->skor_katrol = '';//round($total, 2);
                }
            }
    ...
    public function simpanKoreksi()
    {
        ...
        foreach ($nilais as $nilai) {
            ...
            $jml += $nilai->koreksi;
            $updated[] = [
                'id_soal_siswa' => $nilai->id_soal,
                'nilai_koreksi' => $nilai->koreksi,
                'nilai_otomatis' => 1
            ];
        }

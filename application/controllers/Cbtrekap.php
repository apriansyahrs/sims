...
/*   ________________________________________
    ...
    |________________________________________|
*/
class Cbtrekap extends MY_Controller {
    ...
    public function backupNilai($id_jadwal) {
        ...
        if ($terpakai && $generated) {
            ...
            foreach ($kelass as $kls) {
                if ($kls['kelas_id'] !=null) $arrkelas[] = $kls['kelas_id'];
            }
            ...
            foreach ($soals as $id=>$soal) {
                if ($soal->jenis == '1') {
                    ...
                    $pg2b[] = ['no_soal' => $id, 'jawab' => $soal->jawaban];
                } elseif ($soal->jenis == '3') {
                    $jodb[] = ['no_soal' => $id, 'jawab' => $soal->jawaban];
                } elseif ($soal->jenis == '4') {
                    $isb[] = ['no_soal' => $id, 'jawab' => $soal->jawaban];
                } elseif ($soal->jenis == '5') {
                    $esb[] = ['no_soal' => $id, 'jawab' => $soal->jawaban];
                }
            }

            $soal_kompleks = [
                'tampil'=>$jadwal->tampil_kompleks,
                'bobot'=>$jadwal->bobot_kompleks,
                'jawaban'=>$pg2b
            ];
            $soal_jodohkan = [
                'tampil'=>$jadwal->tampil_jodohkan,
                'bobot'=>$jadwal->bobot_jodohkan,
                'jawaban'=>$jodb
            ];
            $soal_isian = [
                'tampil'=>$jadwal->tampil_isian,
                'bobot'=>$jadwal->bobot_isian,
                'jawaban'=>$isb
            ];
            $soal_essai = [
                'tampil'=>$jadwal->tampil_esai,
                'bobot'=>$jadwal->bobot_esai,
                'jawaban'=>$esb
            ];
            ...
            $insert = [
                ...
                'soal_kompleks' => serialize($soal_kompleks),
                'soal_jodohkan' => serialize($soal_jodohkan),
                'soal_isian'    => serialize($soal_isian),
                'soal_essai'    => serialize($soal_essai),
            ...
            foreach ($siswas as $siswa) {
                ...
                if (isset($nilais[$siswa->id_siswa])) {
                    $dikoreksi[] = $nilais[$siswa->id_siswa]->dikoreksi;
                ...
                if (isset($jawabans[$siswa->id_siswa])) {
                    foreach ($jawabans[$siswa->id_siswa] as $jawaban) {
                        if ($jawaban->jenis_soal == '1') {
                            ...
                            $pg2s[] = ['no_soal' => $jawaban->id_soal, 'jawab' => $jawaban->jawaban_siswa];
                        } elseif ($jawaban->jenis_soal == '3') {
                            $jods[] = ['no_soal' => $jawaban->id_soal, 'jawab' => $jawaban->jawaban_siswa];
                        } elseif ($jawaban->jenis_soal == '4') {
                            $iss[] = ['no_soal' => $jawaban->id_soal, 'jawab' => $jawaban->jawaban_siswa];
                        } elseif ($jawaban->jenis_soal == '5') {
                            $ess[] = ['no_soal' => $jawaban->id_soal, 'jawab' => $jawaban->jawaban_siswa];
                        }
                    }
                }
                $soal_pg2 = [
                ...
                $nilai[] = [
                    ...
                    'soal_kompleks' => serialize($soal_pg2),
                    'soal_jodohkan' => serialize($soal_jod),
                    'soal_isian'    => serialize($soal_is),
                    'soal_essai'    => serialize($soal_es),
                    'id_guru'       => $jadwal->id_guru
                ];
            }
            ...
            //$data['rekap'] = 0;
            //$data['nilai'] = $save;
            //$data['message'] = 'Jadwal Ujian masih berlangsung, '.$save.' nilai siswa berhasil direkap<b>beberapa siswa belum selesai atau dikoreksi';
            //$this->output_json($data);
        }

        if ($generated && $result) {
            $this->session->set_flashdata('rekapnilai',
                '<div id="flashdata" class="alert alert-default-success align-content-center w-100" role="alert"> Berhasil merekap nilai '.$save.' siswa</div>');
        } else {
            $this->session->set_flashdata('rekapnilai',
    ...
    public function bulkBackup() {
            ...
            if ($terpakai) {
                ...
                */
                ...
                foreach ($siswas as $siswa) {
                    ...
                    $nilai[] = [
                        //'id_rekap_nilai' => $siswa->id_siswa.'0'.$id_jadwal,
                        'id_jadwal' => $jadwal->id_jadwal,
                        'id_tp' => $id_tp->id_tp,
                        'tp' => $tahun,
                        'id_smt' => $id_smt->id_smt,
        ...
        //$data['rekap'] = $sukses;
        //$data['nilai'] = $save;
        //$data['message'] = $sukses ? 'Jadwal ujian dan '.$save.' nilai siswa berhasil direkap'
        //    : 'Jadwal Ujian masih berlangsung, '.$save.' nilai siswa berhasil direkap<b>beberapa siswa belum selesai atau dikoreksi';
        //end

        if ($generated > 0 && $result) {
            $this->session->set_flashdata('rekapnilai',
                '<div id="flashdata" class="alert alert-default-success align-content-center w-100" role="alert"> Berhasil merekap <b>'.count($ids).'</b> nilai </div>');
        } else {
            $this->session->set_flashdata('rekapnilai',
                '<div id="flashdata" class="alert alert-default-danger align-content-center w-100" role="alert">Jadwal Ujian masih berlangsung, '.$save.' nilai siswa berhasil direkap.<br>Beberapa siswa belum selesai atau belum dikoreksi</div>');
        }
    ...
    */
    ...
    public function getNilaiKelas() {
        ...
        if($this->ion_auth->is_admin()){
            ...
            foreach ($jadwals as $key=>$jadwal) {
                ...
                $jadwal->jawaban_pg = $this->maybe_unserialize($jadwal->jawaban_pg ?? '');
                $jadwal->jawaban_esai = $this->maybe_unserialize($jadwal->jawaban_esai ?? '');

                $ids = [];
                foreach ($jadwal->bank_kelas as $id) {
                    $ids[] = $id['kelas_id'];
                }
                if (!in_array($kelas, $ids)) unset($jadwals[$key]);
            }
        ...
        if (count($rekaps)>0) {
            foreach ($rekaps as $rekap) {
                $rekap->jawaban_pg = $this->$this->maybe_unserialize_with_key($rekap->jawaban_pg);

                $rekap->soal_kompleks = json_decode(json_encode($this->maybe_unserialize($rekap->soal_kompleks)));
                $rekap->soal_jodohkan = json_decode(json_encode($this->maybe_unserialize($rekap->soal_jodohkan)));
                $rekap->soal_isian = json_decode(json_encode($this->maybe_unserialize($rekap->soal_isian)));
                $rekap->soal_essai = json_decode(json_encode($this->maybe_unserialize($rekap->soal_essai)));

                $arrSiswa[$rekap->id_siswa] = [
                    'id_siswa'=>$rekap->id_siswa,
                    'nomor_peserta'=>$rekap->nomor_peserta,
                    'nama'=>$rekap->nama,
                ];
            }
        }
    ...
    public function olahNilai() {
        ...
        $kls = $this->maybe_unserialize($rekap->nama_kelas);
        $data['kelas'] = $kls;
        ...
        if($this->ion_auth->is_admin()){
            if ($kelas!=null) {
                ...
                foreach ($siswas as $siswa) {
                    ...
                    $siswa->soal_kompleks = json_decode(json_encode($this->maybe_unserialize($siswa->soal_kompleks)));
                    $siswa->soal_jodohkan = json_decode(json_encode($this->maybe_unserialize($siswa->soal_jodohkan)));
                    $siswa->soal_isian = json_decode(json_encode($this->maybe_unserialize($siswa->soal_isian)));
                    $siswa->soal_essai = json_decode(json_encode($this->maybe_unserialize($siswa->soal_essai)));
                }
                $data['siswas'] = $siswas;
            ...
            if ($kelas!=null) {
                ...
                foreach ($siswas as $siswa) {
                    $siswa->jawaban_pg = $this->$this->maybe_unserialize_with_key($siswa->jawaban_pg);
    ...
    function unserialize_with_key($serialized) {
        $arr = $this->maybe_unserialize($serialized);
        $result = [];
    ...
    public function export() {
        ...
        if($this->ion_auth->is_admin()){
            ...
            foreach ($jadwals as $jadwal) {
                $jadwal->bank_kelas = $this->maybe_unserialize($jadwal->bank_kelas);
                $jadwal->nama_kelas = $this->maybe_unserialize($jadwal->nama_kelas);
            }
            $data['rekaps'] = $jadwals;
            $data['profile'] = $this->dashboard->getProfileAdmin($user->id);
            $this->load->view('_templates/dashboard/_header', $data);
    ...
    public function generateNilaiUjian($jadwal) {
        ...
        $kelas_bank = $this->maybe_unserialize($info->bank_kelas ?? '');
        $kelases = [];
        foreach ($kelas_bank as $key => $value) {
            $kelases[] = $value['kelas_id'];
        }

        $siswas = $this->cbt->getSiswaByKelas($info->id_tp, $info->id_smt, $kelases);
        $jawabans = $this->cbt->getJawabanByBank($info->id_bank);
        $unseriali_jawaban = $this->unserializeJawabanSiswa($jawabans);
        $soal = $unseriali_jawaban['soal'];

...
$(document).ready(function () {
  ...
  /*
    ...
    */
  table.buttons().container().appendTo("#jurusan_wrapper .col-md-6:eq(1)");
  ...
  //   $("#createDokumenModal").on("show.bs.modal", function (e) {
  //   });

  $("#editDokumenModal").on("show.bs.modal", function (e) {
    var id = $(e.relatedTarget).data("id");
    var dokumen = $(e.relatedTarget).data("dokumen");
    var link = $(e.relatedTarget).data("link");
    $("#dokumenEdit").val(dokumen);
    $("#linkEdit").val(link);
    $("#editIdDokumen").val(id);
  });
  ...
  $("#create").submit("click", function (e) {
    ...
    $.ajax({
      url: base_url + "datadokumen/store",
      type: "POST",
      dataType: "JSON",
      data: $(this).serialize(),
      success: function (data) {
        console.log("result", data);
        $("#createDokumenModal").modal("hide").data("bs.modal", null);
        $("#createDokumenModal").on("hidden", function () {
  ...
  $("#update").on("submit", function (e) {
    ...
    $.ajax({
      url: base_url + "datadokumen/update",
      data: $(this).serialize(),
      method: "POST",
      dataType: "JSON",
      success: function (data) {
  ...
  $("#bulk").on("submit", function (e) {
    if ($(this).attr("action") == base_url + "datadokumen/delete") {
      e.preventDefault();
      e.stopImmediatePropagation();
...
function bulk_delete(frombody, id) {
  if ($("#dokumen tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "datadokumen/delete");
    swal
      .fire({
        title: "Anda yakin?",
